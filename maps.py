...
def init(screen, background_image_param):
    global width, height, display, background_image, ground_image
    display = screen
    (width, height) = display.get_rect().size
    height -= ground
    interface.init(display)
    background_image = background_image_param
    ground_image = pygame.image.load("Images/ground_texture.jpg").convert_alpha()
...
class Maps:
    ...
    def level_cleared(self):
        ...
        if self.level <= self.max_level:
            level_cleared_text.add_text("LEVEL " + str(self.level - 1) + " CLEARED!", 80, "Fonts/Comic_Kings.ttf", (0, 0, 0))
        else:
            level_cleared_text.add_text("ALL LEVEL CLEARED!", 80, "Fonts/Comic_Kings.ttf", (0, 0, 0))

        score_text = interface.Label(750, 300, 300, 100, None, self.color['background'])
        score_text.add_text("SCORE: " + str(self.score), 55, "Fonts/Comic_Kings.ttf", (0, 0, 0))

        replay = interface.Button(350, 500, 300, 100, self.replay_level, (244, 208, 63), (247, 220, 111))
        replay.add_text("PLAY AGAIN", 60, "Fonts/arfmoochikncheez.ttf", (0, 0, 0))

        if self.level <= self.max_level:
            next = interface.Button(750, 500, 300, 100, self.draw_map, (88, 214, 141), (171, 235, 198))
            next.add_text("CONTINUE", 60, "Fonts/arfmoochikncheez.ttf", (0, 0, 0))
        else:
            next = interface.Button(750, 500, 300, 100, self.start_again, (88, 214, 141), (171, 235, 198))
            next.add_text("START AGAIN", 60, "Fonts/arfmoochikncheez.ttf", (0, 0, 0))
    ...
    def level_failed(self):
        ...
        level_failed_text.add_text("LEVEL FAILED!", 80, "Fonts/Comic_Kings.ttf", (0, 0, 0))

        score_text = interface.Label(750, 300, 300, 100, None, self.color['background'])
        score_text.add_text("SCORE: " + str(self.score), 55, "Fonts/Comic_Kings.ttf", (0, 0, 0))

        replay = interface.Button(500, 500, 300, 100, self.draw_map, (244, 208, 63), (247, 220, 111))
        replay.add_text("TRY AGAIN", 60, "Fonts/arfmoochikncheez.ttf", (0, 0, 0))
    ...
    def start_level(self, birds, pigs, blocks, walls):
        ...
        score_text.add_text("SCORE: " + str(self.score), 25, "Fonts/Comic_Kings.ttf", (0, 0, 0))

        birds_remaining = interface.Label(120, 50, 100, 50, None, self.color['background'])
        birds_remaining.add_text("BIRDS REMAINING: " + str(len(birds)), 25, "Fonts/Comic_Kings.ttf", (0, 0, 0))

        pigs_remaining = interface.Label(110, 90, 100, 50, None, self.color['background'])
        pigs_remaining.add_text("PIGS REMAINING: " + str(len(pigs)), 25, "Fonts/Comic_Kings.ttf", (0, 0, 0))
        ...
        while loop:
            ...
            display.blit(background_image, (0, 0))
            display.blit(ground_image, (0, height))
